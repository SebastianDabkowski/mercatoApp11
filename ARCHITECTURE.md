...
- DocSync agent automatically regenerates `ARCHITECTURE.md` (`architecture.md`) when architecture changes and `AGENT.md` (`agent.md`) when agent responsibilities change, validates all ADRs against the current architecture, and creates any missing ADR stubs when validation runs.
- DocSync validation is required on every architecture change: regenerate `ARCHITECTURE.md` and `AGENT.md`, verify ADR coverage, and auto-create missing ADRs before merge to keep docs in lockstep with implementation.
