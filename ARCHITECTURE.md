...
- Run doc-sync automation on every architecture change to refresh `ARCHITECTURE.md` and `AGENT.md` from the implemented module inventory.
- When an ADR is added or its status changes, add it to ADR Traceability and ensure the record exists in `docs/adr`.
- Run ADR validation before merge; validate ADR coverage against the current architecture, create any missing ADR, and update ADR Traceability/AGENT.md in the same PR.
